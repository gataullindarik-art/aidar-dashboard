...
def process_wb_data(sales, orders):
    ...
    try:
        # Подсчитываем метрики вручную (без pandas)
        total_revenue = sum(s.get('salePrice', 0) for s in sales)
        total_quantity = len(sales)
        unique_products = len(set(s.get('nmId') for s in sales if 'nmId' in s))
        ...
        # Подготавливаем дневные данные (группируем вручную)
        daily_by_date = defaultdict(lambda: {'revenue': 0, 'count': 0})
        for sale in sales:
            if 'saleDt' in sale:
                date = sale['saleDt'].split('T')[0]  # Извлекаем только дату
                daily_by_date[date]['revenue'] += sale.get('salePrice', 0)
                daily_by_date[date]['count'] += 1
        
        for date in sorted(daily_by_date.keys()):
            report['daily_data'].append({
                'date': date,
                'revenue': float(daily_by_date[date]['revenue']),
                'orders': int(daily_by_date[date]['count'])
            })
